...
def load_tablets_from_supabase():
    ...
    try:
        ...
        products_result = supabase.table('products').select('*').eq('category', 'tablet').eq('is_active', True).execute()
        ...
        prices_result = supabase.table('price_history').select('*').execute()
...
def load_mobiles_from_supabase():
    try:
        ...
        products_result = supabase.table('products').select('*').eq('category', 'mobile').eq('is_active', True).execute()
...
def load_and_preprocess_data(filepath='tablets'):
    ...
    if 'tablet' in filepath.lower():
        ...
        raise ValueError(f"Unknown filepath: {filepath}. Use 'tablets' or 'mobiles'")
