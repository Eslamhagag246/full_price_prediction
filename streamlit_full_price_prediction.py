...
if AUTO_REFRESH_AVAILABLE:
    tick = st_autorefresh(interval=AUTO_REFRESH_SECONDS * 1000, key="auto_refresh_data")
...
# ═══════════════════════════════════════════════════════════
# CUSTOM CSS - KEEP ORIGINAL COLORS
# ═══════════════════════════════════════════════════════════
st.markdown("""
<style>
...
def _sort_history(pdf: pd.DataFrame) -> pd.DataFrame:
    pdf = pdf.copy()
...
def _get_actual_current_info(pdf: pd.DataFrame):
    pdf = _sort_history(pdf)
    if pdf.empty:
        return np.nan, None, ""
    latest = pdf.iloc[-1]
    current_price = pd.to_numeric(latest.get("price"), errors="coerce")
    try:
        current_price = float(current_price)
    except Exception:
        current_price = np.nan
    return current_price, latest.get("date", None), latest.get("URL", "")

def _get_final_forecast_price(result) -> float:
    series = pd.to_numeric(pd.Series(result.get("forecast_prices", [])), errors="coerce").dropna()
    if series.empty:
        return np.nan
...
def _category_from_device_type(device_type: str) -> str:
    return "tablet" if device_type == "Tablets" else "mobile"

def generate_buy_signal(result, actual_current_price):
    last_price = actual_current_price
    future_price = _get_final_forecast_price(result)

    if pd.isna(last_price) or pd.isna(future_price) or last_price <= 0:
        return {
            'type': 'hold',
            'icon': '🟡',
            'title': 'HOLD/NEUTRAL',
            'desc': 'Not enough reliable data to compute the signal',
            'detail': 'Please check latest product history rows',
...
if st.session_state.show_market_insights:
    ...
    for product_key in df['product_key'].unique():
        pdf = _sort_history(df[df['product_key'] == product_key].copy())
        if len(pdf) < 2:
            continue
    ...
    if price_changes:
        ...
        with col1:
            ...
            if not drops.empty:
                display_drops = drops[['Product', 'Website', 'Change %', 'Current Price']]
                st.dataframe(display_drops, use_container_width=True, hide_index=True)
            else:
        ...
        with col2:
            ...
            if not rises.empty:
                display_rises = rises[['Product', 'Website', 'Change %', 'Current Price']]
                st.dataframe(display_rises, use_container_width=True, hide_index=True)
            else:
...
with col1:
    brands = sorted([b for b in filtered_df['brand'].dropna().unique() if str(b).strip() != ""])
...
if app_mode == "🔮 Price Forecast":
    ...
    for product_key in selected_products:
        ...
        current_price, current_date, current_url = _get_actual_current_info(product_df)
        actual_current_map[product_key] = {
            "price": current_price,
            "date": current_date,
            "url": current_url
        }
    ...
    if compare_mode:
        ...
        for info, result in zip(product_infos, results):
            actual_current_price = actual_current_map[info['product_key']]["price"]
            final_forecast_price = _get_final_forecast_price(result)
        ...
        current_info = actual_current_map[product_info['product_key']]

        actual_current_price = current_info["price"]
        actual_url = current_info["url"]
        final_forecast_price = _get_final_forecast_price(result)
        ...
        current_text = f"EGP {signal['current']:,.0f}" if pd.notna(signal['current']) else "N/A"
        forecast_text = f"EGP {signal['forecast']:,.0f}" if pd.notna(signal['forecast']) else "N/A"
        ...
        <div class="signal-banner signal-{signal['type']}">
            ...
            <div class="signal-detail" style="margin-top: 0.5rem;">
                Current: {current_text} → Forecast: {forecast_text} |
        ...
        with stat_col3:
            egp_text = f"{expected_change_egp:+,.0f} EGP" if pd.notna(expected_change_egp) else "N/A"
            pct_text = f"({expected_change_pct:+.1f}%)" if pd.notna(expected_change_pct) else ""
            st.markdown(f"""
            <div class="stat-card">
                <div class="stat-label">Expected Change</div>
                <div class="stat-value">{egp_text}</div>
                <div style="font-size:0.9rem; margin-top:0.3rem;">{pct_text}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        forecast_df = pd.DataFrame({
            ...
            'Lower Bound (EGP)': [max(0, p - result['mae']) for p in pd.to_numeric(pd.Series(result['forecast_prices']), errors='coerce').fillna(0)],
            'Upper Bound (EGP)': [p + result['mae'] for p in pd.to_numeric(pd.Series(result['forecast_prices']), errors='coerce').fillna(0)]
        })
        ...
        forecast_table = pd.DataFrame({
            ...
            'Forecasted Price': [f"EGP {p:,.0f}" for p in pd.to_numeric(pd.Series(result['forecast_prices']), errors='coerce').fillna(0)],
            'Lower Bound': [f"EGP {max(0, p - result['mae']):,.0f}" for p in pd.to_numeric(pd.Series(result['forecast_prices']), errors='coerce').fillna(0)],
            'Upper Bound': [f"EGP {(p + result['mae']):,.0f}" for p in pd.to_numeric(pd.Series(result['forecast_prices']), errors='coerce').fillna(0)]
        })
    ...
    with st.spinner("🔍 Analyzing prices across websites..."):
        recommendation = get_product_recommendation(
            ...
            category=_category_from_device_type(device_type),
            df=df
        )
