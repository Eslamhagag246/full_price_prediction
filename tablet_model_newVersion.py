...
def load_and_preprocess_data(filepath='tablets_cleaned_continuous.csv'):
    """Load and preprocess tablet data"""
    df = pd.read_csv(filepath)

    # Clean price
    df['price'] = df['price'].astype(str).str.replace('EGP', '', regex=False).str.replace(',', '', regex=False).str.strip()
    df['price'] = pd.to_numeric(df['price'], errors='coerce')
    df = df.dropna(subset=['price'])
    ...
    return df_daily.sort_values(['product_key', 'date'])
...
def engineer_features(pdf):
    ...
    # Rolling features - use fillna instead of shift to avoid NaN
    pdf['rolling_avg_3'] = pdf['price'].rolling(3, min_periods=1).mean().fillna(pdf['price'])
    pdf['rolling_avg_7'] = pdf['price'].rolling(7, min_periods=1).mean().fillna(pdf['price'])
    pdf['rolling_std_3'] = pdf['price'].rolling(3, min_periods=1).std().fillna(0)

    # Lag features - use fillna to avoid NaN
    ...
    # ✅ CRITICAL: Create pct_change BEFORE any dropna()
    pdf['pct_change_1'] = pdf['price'].pct_change().fillna(0)
    pdf['pct_change_3'] = pdf['price'].pct_change(3).fillna(0)
...
def train_global_model(filepath='tablets_cleaned_continuous.csv', min_obs=10, test_size=0.2):
    """Train global model on all products"""
    df = load_and_preprocess_data(filepath)
    ...
    for product_key in df['product_key'].unique():
        ...
        # Now we can safely check for required columns
        if not all(col in pdf.columns for col in FEATURE_COLS):
            print(f"   ⚠️ Skipping {product_key} - missing features")
            continue
...
def forecast_product(pdf, days_ahead=7, model=None):
    ...
    missing = [col for col in FEATURE_COLS if col not in pdf.columns]
    if missing:
